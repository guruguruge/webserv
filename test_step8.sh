...
RESPONSE=$(echo "$TEST_CONTENT" | curl -s -H "Connection: close" -X POST http://$HOST:$PORT/upload -d @- -w "\n%{http_code}")
...
RESPONSE=$(curl -s -H "Connection: close" -X POST http://$HOST:$PORT/upload \
...
RESPONSE=$(curl -s -H "Connection: close" -X DELETE http://$HOST:$PORT/upload/test_delete.txt -w "\n%{http_code}")
STATUS=$(echo "$RESPONSE" | tail -n 1)
...
RESPONSE=$(curl -s -H "Connection: close" -X DELETE http://$HOST:$PORT/upload/nonexistent.txt -w "\n%{http_code}")
STATUS=$(echo "$RESPONSE" | tail -n 1)
...
RESPONSE=$(curl -s -H "Connection: close" -X POST http://$HOST:$PORT/ -d "test" -w "\n%{http_code}")
STATUS=$(echo "$RESPONSE" | tail -n 1)
...
RESPONSE=$(curl -s -H "Connection: close" -X DELETE http://$HOST:$PORT/ -w "\n%{http_code}")
STATUS=$(echo "$RESPONSE" | tail -n 1)
...
RESPONSE=$(curl -s -H "Connection: close" -X DELETE http://$HOST:$PORT/upload/testdir -w "\n%{http_code}")
STATUS=$(echo "$RESPONSE" | tail -n 1)
...
RESPONSE=$(echo "test" | curl -s -H "Connection: close" -X POST http://$HOST:$PORT/upload -d @- -i)
...
RESPONSE=$(curl -s -H "Connection: close" -X DELETE http://$HOST:$PORT/upload/test_delete2.txt -w "\n%{http_code}")
BODY=$(echo "$RESPONSE" | sed '$d')
STATUS=$(echo "$RESPONSE" | tail -n 1)
[ "$STATUS" = "204" ] && [ -z "$BODY" ]
test_result
...
RESPONSE=$(curl -s -H "Connection: close" http://$HOST:$PORT/upload/)
