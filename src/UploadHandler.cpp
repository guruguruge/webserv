...
HttpResponse UploadHandler::handlePost(const HttpRequest& req,
    ...
    if (!location) {
        return ErrorPageManager::makeErrorResponse(404, &server, "Location not found");
    }

    // upload_path が設定されていない場合はエラー
    if (location->uploadPath.empty()) {
        return ErrorPageManager::makeErrorResponse(403, &server, "Upload not allowed");
    }

    // client_max_body_size のチェック
    if (server.clientMaxBodySize > 0 && req.body.size() > server.clientMaxBodySize) {
        return ErrorPageManager::makeErrorResponse(413, &server, "Request entity too large");
    }
...
HttpResponse UploadHandler::handleMultipart(const HttpRequest& req,
    ...
    while (pos < body.size()) {
        ...
        // 次の delimiter を探す
        size_t nextDelimiterPos = body.find(delimiter, delimiterPos + delimiter.size());
        if (nextDelimiterPos == std::string::npos) {
            break;
        }

        // このパートを抽出
        std::string part = body.substr(delimiterPos + delimiter.size(), 
    ...
    if (filesUploaded > 0) {
        ...
        oss << "<!DOCTYPE html>\n"
            ...
            << "<p>" << filesUploaded << " file(s) uploaded: " << uploadedFiles << "</p>\n"
            << "</body>\n"
            << "</html>\n";
...
std::string UploadHandler::extractBoundary(const std::string& contentType) {
    ...
    // 前後の空白を削除
    size_t start = 0;
    while (start < boundary.size() && (boundary[start] == ' ' || boundary[start] == '\t')) {
        start++;
    }
    size_t finish = boundary.size();
    while (finish > start && (boundary[finish - 1] == ' ' || boundary[finish - 1] == '\t')) {
        finish--;
    }
...
bool UploadHandler::saveFile(const std::string& path, const std::string& content) {
    // ディレクトリが存在するか確認
