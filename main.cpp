...
struct EpollContext
{
    ...
    EpollContext(int f, int port) : fd(f), is_listener(true), client(NULL), listen_port(port) {}
    EpollContext(Client *c) : fd(c->getFd()), is_listener(false), client(c), listen_port(0) {}
};
...
void processRequest(Client *client, const MainConfig &mainConfig)
{
    ...
    if (req.getMethod() == GET)
    {
        ...
        std::string body = "<html><body><h1>Hello from Webserv!</h1><p>Path: " + req.getPath() + "</p></body></html>";
        res.setStatusCode(200);
        res.setHeader("Content-Type", "text/html");
        res.setHeader("Content-Length", "wait to build"); // build時に自動計算される想定なら不要
        res.setBody(body);
    }
    else
    {
    ...
    while (true)
    {
        ...
        for (int i = 0; i < nfds; ++i)
        {
            ...
            if (ctx->is_listener)
            {
                ...
                int conn_fd = accept(ctx->fd, (struct sockaddr *)&client_addr, &client_len);
