...
class CgiHandler {
 public:
  /**
     ...
     */
  static bool startCgi(ClientConnection& client, const HttpRequest& req,
 ...
 private:
  /**
     * CGI用の環境変数を構築
     */
  static char** buildEnvp(const HttpRequest& req, const ServerConfig& server,
                          const LocationConfig& location,
  ...
  /**
     * スクリプトファイルのパスを解決
     */
  static std::string resolveScriptPath(const HttpRequest& req,
                                       const LocationConfig& location);
};
