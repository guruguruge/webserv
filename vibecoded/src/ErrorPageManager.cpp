...
HttpResponse ErrorPageManager::makeErrorResponse(
    int statusCode, const ServerConfig* serverConfig,
  ...
  if (serverConfig) {
    std::map<int, std::string>::const_iterator it =
        serverConfig->errorPages.find(statusCode);
  ...
  if (!customPageLoaded) {
    std::string displayMessage =
        message.empty() ? response.getReasonPhrase() : message;
    body = generateDefaultErrorPage(statusCode, response.getReasonPhrase(),
                                    displayMessage);
    response.setHeader("Content-Type", "text/html");
  }
...
bool ErrorPageManager::readErrorPage(const std::string& filePath,
                                     std::string& content) {
  // ファイルの存在確認
  struct stat st;
