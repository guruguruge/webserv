...
bool HttpRequestParser::parseRequestLine() {
    ...
    // HTTPバージョン検証
    if (_request.httpVersion != "HTTP/1.1" && _request.httpVersion != "HTTP/1.0") {
...
bool HttpRequestParser::parseHeaders() {
    while (true) {
        ...
        if (line.empty()) {
            ...
            std::map<std::string, std::string>::const_iterator it = _request.headers.find("transfer-encoding");
            if (it != _request.headers.end() && it->second.find("chunked") != std::string::npos) {
...
bool HttpRequestParser::parseBody() {
    ...
    std::map<std::string, std::string>::const_iterator it = _request.headers.find("content-length");
    if (it == _request.headers.end()) {
...
bool HttpRequestParser::parseChunkData() {
    ...
    // チャンクデータを完全に読み込む
    _request.body.insert(_request.body.end(), _buffer.begin(), _buffer.begin() + remaining);
