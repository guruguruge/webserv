...
void Server::acceptNewClient(int listenFd) {
  ...
  ClientConnection* client =
      new ClientConnection(clientFd, listener->getServerConfig());
...
void Server::handleClientRead(int clientFd) {
  ...
  if (n > 0) {
    std::cout << "📖 Read " << n << " bytes from client (fd: " << clientFd
              << ")" << std::endl;
    ...
    if (parser.getState() == HttpRequestParser::PARSE_DONE) {
      ...
      if (serverConfig) {
        ...
        std::cout << "   Server: "
                  << (serverConfig->serverName.empty()
                          ? "(default)"
                          : serverConfig->serverName)
                  << std::endl;
        std::cout << "   Location: "
                  << (locationConfig ? locationConfig->path : "(none)")
                  << std::endl;
      }
      ...
      if (locationConfig && !locationConfig->cgiExtension.empty()) {
        ...
        if (dotPos != std::string::npos) {
          ...
          std::cout << "🔍 Checking CGI: ext=" << ext
                    << ", cgiExtension=" << locationConfig->cgiExtension
                    << std::endl;
      ...
      if (!serverConfig) {
        ...
        response = ErrorPageManager::makeErrorResponse(500, NULL,
                                                       "Internal Server Error");
      } else if (isCgi) {
        // CGIで処理
        bool cgiStarted =
            CgiHandler::startCgi(*client, req, *serverConfig, *locationConfig);
        ...
        // 静的ファイルハンドラーで処理
        response =
            StaticFileHandler::handleGet(req, *serverConfig, locationConfig);
      } else if (req.method == "POST") {
        // アップロードハンドラーで処理
        response =
            UploadHandler::handlePost(req, *serverConfig, locationConfig);
      } else if (req.method == "DELETE") {
        // 静的ファイルハンドラーで削除処理
        response =
            StaticFileHandler::handleDelete(req, *serverConfig, locationConfig);
      } else {
        // その他のメソッドは未実装
        response = ErrorPageManager::makeErrorResponse(501, serverConfig,
                                                       "Not Implemented");
      }
      ...
      // パースエラー
      std::cerr << "❌ HTTP parse error: " << parser.getErrorMessage()
                << std::endl;

      // 400 Bad Request を返す
      // サーバー設定を取得して、カスタムエラーページを使用
      const ServerConfig* serverConfig =
          Router::findServer(_config, parser.getRequest());
      HttpResponse response =
          ErrorPageManager::makeErrorResponse(400, serverConfig, "Bad Request");
      ...
      // パース継続中 - 次のデータを待つ
      std::cout << "   Waiting for more data (current state: "
                << parser.getState() << ")" << std::endl;
    }
  } else if (n == 0) {
    // クライアントが接続を閉じた
    ...
    if (errno != EAGAIN && errno != EWOULDBLOCK) {
      std::cerr << "⚠️  Read error on client (fd: " << clientFd << ")"
                << std::endl;
      closeClient(clientFd);
    }
  }
...
void Server::handleClientWrite(int clientFd) {
  ...
  if (n > 0) {
    std::cout << "📝 Wrote " << n << " bytes to client (fd: " << clientFd << ")"
              << std::endl;

    // 全部送信完了したか確認
    if (client->getSendBuffer().empty()) {
      std::cout << "✅ Response sent completely to client (fd: " << clientFd
                << ")" << std::endl;

      // keep-alive のチェック
      if (client->isKeepAlive()) {
        // keep-alive: 接続を保持して次のリクエストを待つ
        std::cout << "🔄 Keep-alive: waiting for next request (fd: " << clientFd
                  << ")" << std::endl;
        ...
        std::cout << "🔚 Connection: close (fd: " << clientFd << ")"
                  << std::endl;
        closeClient(clientFd);
      }
    }
    ...
    // エラー（EAGAINやEWOULDBLOCKは正常）
    if (errno != EAGAIN && errno != EWOULDBLOCK) {
      std::cerr << "⚠️  Write error on client (fd: " << clientFd << ")"
                << std::endl;
      closeClient(clientFd);
    }
  }
...
void Server::closeClient(int clientFd) {
  ...
  std::cout << "❌ Client connection closed (fd: " << clientFd << ")"
            << std::endl;
}
