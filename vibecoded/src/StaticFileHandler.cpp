...
HttpResponse StaticFileHandler::handleGet(
  ...
  if (request.method != "GET" && request.method != "HEAD") {
    return ErrorPageManager::makeErrorResponse(405, &serverConfig,
                                               "Method Not Allowed");
  }
    ...
    if (!locationConfig->index.empty()) {
      ...
      if (fileExists(indexPath) && !isDirectory(indexPath)) {
        ...
        if (readFile(indexPath, content)) {
          ...
          return ErrorPageManager::makeErrorResponse(500, &serverConfig,
...
std::string StaticFileHandler::buildFilePath(
    const LocationConfig* locationConfig, const std::string& requestPath) {
  if (!locationConfig) {
    return requestPath;
  }
...
bool StaticFileHandler::readFile(const std::string& path,
                                 std::string& content) {
...
std::string StaticFileHandler::generateDirectoryListing(
    const std::string& dirPath, const std::string& requestPath) {
  ...
  // 親ディレクトリへのリンク
  if (requestPath != "/") {
    html << "    <li><a href=\"../\" class=\"dir\">[Parent "
            "Directory]</a></li>\n";
  }
...
std::vector<std::string> StaticFileHandler::listDirectory(
...
HttpResponse StaticFileHandler::handleDelete(
  ...
  // ディレクトリの場合はエラー
  if (isDirectory(filePath)) {
    return ErrorPageManager::makeErrorResponse(403, &serverConfig,
