...
// ヘルパー
// =============================================================================
void setupClientRequest(Client& client, const std::string& method, const std::string& path, const std::string& body = "") {
    client.req.clear();
    client.res.clear();
...
// CGI実行結果の検証
void assertCgiExecution(Client& client, const std::string& testName, const std::string& expectedOutputPart) {
    // 1. 状態遷移の確認
    if (client.getState() != WAITING_CGI) {
        std::cout << RED << "[FAIL] " << testName 
                  << " | Expected State: WAITING_CGI, Actual: " << client.getState() << RESET << std::endl;
        return;
    }
    ...
    if (client.getCgiPid() <= 0 || client.getCgiStdoutFd() < 0) {
        std::cout << RED << "[FAIL] " << testName 
                  << " | Invalid PID or FD. PID=" << client.getCgiPid() << RESET << std::endl;
        return;
    }
    ...
    if (output.find(expectedOutputPart) != std::string::npos) {
        ...
        std::cout << RED << "[FAIL] " << testName 
                  << " | Expected output containing: '" << expectedOutputPart << "'" << RESET << std::endl;
    ...
    // --- TEST 3: CGI 404 (存在しないスクリプト) ---
    {
        ...
        if (client.getState() == WRITING_RESPONSE && client.res.getData()) {
            ...
            if (res.find("404 Not Found") != std::string::npos) {
                ...
                std::cout << RED << "[FAIL] CGI 404 | Status mismatch" << RESET << std::endl;
            }
        } else {
             std::cout << RED << "[FAIL] CGI 404 | State mismatch or No Data" 
                       << " State: " << client.getState() 
                       << " Data: " << (client.res.getData() ? "OK" : "NULL") << RESET << std::endl;
        }
    }
