...
void assertResponse(Client& client, int expectedStatus,
  ...
  if (resData.find(expectedStr) != std::string::npos) {
    ...
    std::string statusLine = (crlf != std::string::npos)
                                 ? resData.substr(0, crlf)
                                 : "Invalid Response";
...
int main() {
  ...
  if (access("test_www/secret.txt", R_OK) == 0) {
    std::cout << YELLOW
              << "[SKIP] GET /secret.txt (Skipping 403 test: Unable to remove "
                 "read permission in this environment)"
              << RESET << std::endl;
  } else {
  ...
  handler.handle(&client);
  assertResponse(client, 404,
                 "GET /../../etc/passwd (Path Traversal Prevention)");
